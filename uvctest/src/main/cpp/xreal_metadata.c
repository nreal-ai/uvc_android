...
FRAME_META_DATA *
get_metadata_ptr_by_frame(uint8_t *raw_frame_data, CV_CAMERA_ID camera_id, int width, int height) {
    FRAME_META_DATA *meta_data_ptr = NULL;

    if (camera_id == CAMERA_CV0_ID) {
        meta_data_ptr = (FRAME_META_DATA *) (raw_frame_data + width * (height - 2));
    } else if (camera_id == CAMERA_CV1_ID) {
        meta_data_ptr = (FRAME_META_DATA *) (raw_frame_data + width * (height - 1));
    } else {
...
//    printf("CV%d current frame id=%d, frame timestamp=%lu ns, host timestamp=%lu ns\n", camera_id, meta_data->frame_id, meta_data->timestamp, nanoseconds);
    ...
    if (camera_id == CAMERA_CV0_ID) {
        if (meta_data->frame_id - cv0_last_frame_id != 1) {
            printf("[Warning]Camera%d lost frame, last frame id: %d current frame id:%d total lost frame:%d\n", \
...
void check_frame_timestamp(FRAME_META_DATA *meta_data, CV_CAMERA_ID camera_id) {
    ...
    if (camera_id == CAMERA_CV0_ID) {
        if ((cv0_last_timestamp) &&
            (meta_data->timestamp - cv0_last_timestamp > MAX_TIMESTAMP_DIFF_NS)) {
            printf("[Warning]Camera%d frame:[%d], last frame timestamp: %lu ns, current frame timestamp:%lu ns, time diff is:%lu ns\n", \
            camera_id, meta_data->frame_id, cv0_last_timestamp, meta_data->timestamp,
                   meta_data->timestamp - cv0_last_timestamp);
        }
        cv0_last_timestamp = meta_data->timestamp;
    } else if (camera_id == CAMERA_CV1_ID) {
        if ((cv1_last_timestamp) &&
            (meta_data->timestamp - cv1_last_timestamp > MAX_TIMESTAMP_DIFF_NS)) {
            printf("[Warning]Camera%d frame:[%d], last frame timestamp: %lu ns, current frame timestamp:%lu ns, time diff is:%lu ns\n", \
            camera_id, meta_data->frame_id, cv1_last_timestamp, meta_data->timestamp,
                   meta_data->timestamp - cv1_last_timestamp);
        }
