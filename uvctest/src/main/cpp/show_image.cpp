...
void ShowImage::push(const NRGrayscaleCameraFrameData *frame) {
    ...
    cam_queue_.PushNonBlockingDroppingIfFull(ptr, kMaxCamSize_, old_ptr);
}


void ShowImage::start(RECORD_SAVE_TYPE type, const std::string& path, const std::string& cam0_file_path, const std::string& cam1_file_path, const std::string& ext_param) {
...
void ShowImage::init(const std::string& path, const std::string& cam0_file_path, const std::string& cam1_file_path, const std::string& ext) {
  //std::cout << __PRETTY_FUNCTION__ << std::endl;
  time_t t;
  time(&t);
  ...
  if (!cam1_file_path.empty()) {
    ...
    if (0 != createDir(std::string(dir_name_) + std::string("/" + cam1_file_path) + "/images/")) {
        exit(-1);
    }
  }
...
void ShowImage::recordCamera() {
  ...
  cam_01_sync_check_txt << "frame_id,image_filename,cam0_exposure_start_time_device,cam0_exposure_duration,cam0_rolling_shutter_time,cam0_gain,cam1_exposure_start_time_device,cam1_exposure_duration,cam1_rolling_shutter_time,cam1_gain,host_notify_time_nanos"<< "\n";
  ...
  while (record_thread_running_ || cam_queue_.Size() > 0) {
    ...
    for (int i = 0; i < ptr->camera_count; i++) {
      ...
      if (ptr->camera_count > 1) {
        ...
        hmd_hw_exposure_middle_time = ptr->cameras[i].exposure_start_time_device + (ptr->cameras[i].exposure_duration/2) + (ptr->cameras[i].rolling_shutter_time/2);
        exposure_end_time_device = ptr->cameras[i].exposure_start_time_device + ptr->cameras[i].exposure_duration + ptr->cameras[i].rolling_shutter_time;
        if(ptr->cameras[i].exposure_start_time_system == 0) {
          hmd_host_exposure_middle_time = 0;
        }else{
          hmd_host_exposure_middle_time = ptr->cameras[i].exposure_start_time_system + (ptr->cameras[i].exposure_duration/2) + (ptr->cameras[i].rolling_shutter_time/2);
        }
        time_stamp_name = cam_file_path_[i] + std::string("timestamps.txt");
        metadata_name = cam_file_path_[i] + std::string("metadata.txt");
      ...
      if(false == only_save_metadata_) {
        ...
        if(ptr->pixel_format == NR_GRAYSCALE_CAMERA_PIXEL_FORMAT_RGB_BAYER_8BPP) {
          ...
          cv::Mat cv_image = cv::Mat(ptr->cameras[i].height + ptr->cameras[i].height/2, ptr->cameras[i].width, CV_8UC1,
                                   (void *)((uint8_t *)ptr->data + ptr->cameras[i].offset), ptr->cameras[i].stride);
          // YUV420P/I420转BGR
          cv::Mat bgrImg;
          ...
          cv::Mat cv_image = cv::Mat(ptr->cameras[i].height, ptr->cameras[i].width, CV_8UC1,
                                     (void *)((uint8_t *)ptr->data + ptr->cameras[i].offset), ptr->cameras[i].stride);
    ...
    cam_01_sync_check_txt << ptr->frame_id << "," << tmp << "," << ptr->cameras[0].exposure_start_time_device <<"," << ptr->cameras[0].exposure_duration <<"," << ptr->cameras[0].rolling_shutter_time << "," << ptr->cameras[0].gain  << "," << ptr->cameras[1].exposure_start_time_device <<"," << ptr->cameras[1].exposure_duration <<"," << ptr->cameras[1].rolling_shutter_time << "," << ptr->cameras[1].gain << "," << ptr->notify_time_nanos << "\n";
    cam_01_sync_check_txt.flush();
